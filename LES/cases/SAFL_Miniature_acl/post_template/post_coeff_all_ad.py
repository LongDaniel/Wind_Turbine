...
#plt.rc('font', family='Arial')
line_uref = ax2_1.plot(data2[:,0],data2[:,3],'--',label='$U_{ref}$')
line_urefmean = ax2_1.plot(data2[:,0], data2[:,4],'-',label=r'$\left<U_{ref}\right>_{t}$')
line_udisk = ax2_1.plot(data2[:,0],data2[:,1],'--',label='$U_{disk}$')
line_udiskmean = ax2_1.plot(data2[:,0], data2[:,2],'-',label=r'$\left<U_{disk}\right>_{t}$')
ax2_1.legend(bbox_to_anchor=(0.95, 0.3))
#ax1.legend(handles=[line_thrust,line_power])
#plt.axis([0,40,0,1])
#plt.title(r'$U_{ref}$ time-history')
plt.xlabel('$t$')
