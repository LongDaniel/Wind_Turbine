...
data_u_local = np.zeros((n_ti/size, ny, nx))
data_v_local = np.zeros((n_ti/size, ny, nx))
data_w_local = np.zeros((n_ti/size, ny, nx))

data_u_output = np.zeros((n_ti, 3))  # time, iz
data_u_output_local = np.zeros((n_ti/size, 3))

index = np.zeros((ny, nx), dtype=np.int)

data_velturb_local = np.zeros((n_ti/size, ny, nx, 3))
...
data_uutemp_local = np.zeros((n_ti/size, nx))
data_uusum_local = np.zeros((ny, 3, 3))
data_uusum_global = np.zeros((ny, 3, 3))
  
data_uu_mean = np.zeros((ny, 3, 3))

# data4: mean profile
data_umean = np.zeros(ny)
data_vmean = np.zeros(ny)
data_wmean = np.zeros(ny)

data_usum_global = np.zeros(ny)
data_vsum_global = np.zeros(ny)
data_wsum_global = np.zeros(ny)
  
data_usum_local = np.zeros(ny)
data_vsum_local = np.zeros(ny)
data_wsum_local = np.zeros(ny)

data_umean_2d = np.zeros((ny, nx))
data_vmean_2d = np.zeros((ny, nx))
data_wmean_2d = np.zeros((ny, nx))

data_usum_2d_global = np.zeros((ny, nx))
data_vsum_2d_global = np.zeros((ny, nx))
data_wsum_2d_global = np.zeros((ny, nx))
  
data_usum_2d_local = np.zeros((ny, nx))
data_vsum_2d_local = np.zeros((ny, nx))
data_wsum_2d_local = np.zeros((ny, nx))

print('Memory assigned at cpu '+str(rank)+'/'+str(size))

# data5: timehistory of certain height
iy = 32
iy_array = np.array([31])
...
for i in range(n_ti/size):
  ...
  fname = "POST_U_2D2_"+"{:010d}".format(ti)+"_"+"{:04d}".format(isec)+".DAT"
...
for i in range(ny):
  if rank==0:
    sys.stdout.write("\rTime averaging at iz= {:d}/{:d}".format(i+1,ny))
    #time.sleep(0.1)
    sys.stdout.flush()
...
for i in range(ny):
  if rank==0:
    sys.stdout.write("\rCalculate fluctuation at iz= {:d}/{:d}".format(i+1,ny))
    #time.sleep(0.1)
    sys.stdout.flush()
...
data_uu_mean[:,:,:] = data_uusum_global[:,:,:] / n_ti / nx
...
data_u_output_local = data_u_local[:, iy_array, ix0]
#if(rank==0):
...
data_velturb_output_local = data_velturb_local[:,iy,:,:]
...
#line_u_z3 = ax1_3.plot(time, data_u[:,iz+5])
  ...
  line_u_y1 = ax1_1.plot(time, data_u_output[:, 0],label='y='+'{:0.3f}'.format(y[iy_array[0]]))
  ...
  plt.savefig('fig_1_instantaneous_various_y.png')
...
## Mean vs y
  ...
  line_umean_y = ax2_1.plot(data_umean, y, label='U')
  line_vmean_y = ax2_1.plot(data_vmean, y, label='V')
  line_wmean_y = ax2_1.plot(data_wmean, y, label='W')
  ax2_1.legend()
  plt.title('Mean velocity vs. y')
  plt.xlabel('Mean Velocity')
  plt.ylabel('y')

  um_hub = np.interp(0.36, y, data_umean)
  print("Um_hub="+str(um_hub))

  plt.savefig('fig_2_mean_y.png')
...
#plt.rc('font', family='Arial')
  ...
  line_uu_y = ax3_1.plot(data_uu_mean[:,0,0], y, label=r"$\overline{u^\prime u^\prime}$")
  line_vv_y = ax3_1.plot(data_uu_mean[:,1,1], y, label=r"$\overline{v^\prime v^\prime}$")
  line_ww_y = ax3_1.plot(data_uu_mean[:,2,2], y, label=r"$\overline{w^\prime w^\prime}$")
  line_uw_y = ax3_1.plot(-data_uu_mean[:,0,2], y, label=r"$\overline{-u^\prime w^\prime}$")
...
#line_uw_z = ax3_1.plot(data_uu_mean[:,0,2], z, label=r"$u'w'$")
  ...
  plt.savefig('fig_3_2ndmoment_y.png')
...
## Mean vs yz
  ...
  xtemp = (x-0.3)/0.15
  ytemp = (y-0.36)/0.15
  X, Y = np.meshgrid(xtemp, ytemp, indexing='ij')
...
## save mean_2d to tecplot format
  fid = open("umean_2d_xy.dat",'w')
  fid.write('VARIABLES = X, Y, Z, U, V, W\n')
  fid.write('ZONE T="'+str(ti_end * dt)+'" I='+str(nx)+' J='+str(ny)+' K='+str(nz)+' SOLUTIONTIME='\
    +str(ti_end * dt)+'\n')
  for k in range(ny):
    for i in range(nx):
      fid.write(str(x[i])+' '+str(y[k])+' '+str(z)+' '+str(data_umean_2d[k,i])\
        +' '+str(data_vmean_2d[k,i])+' '+str(data_wmean_2d[k,i])+'\n')
  fid.close()

## Save data
  np.savez('result_mean',y,data_umean, data_vmean, data_wmean, data_uu_mean, \
    data_umean_2d, data_vmean_2d, data_wmean_2d, x, z)

  np.savez('result_timehistory', y, data_u_output, iy, iy_array, time, data_velturb_output)
