...
n_ti = int((ti_end-ti_start)/ti_interval) + 1
n_ti_1 = int(n_ti / size)
...
time_local = np.zeros(n_ti_1)
data_u_local = np.zeros((n_ti_1, nz, ny))
data_v_local = np.zeros((n_ti_1, nz, ny))
data_w_local = np.zeros((n_ti_1, nz, ny))

data_u_output = np.zeros((n_ti, 3))  # time, iz
data_u_output_local = np.zeros((n_ti_1, 3))

index = np.zeros((nz, ny), dtype=np.int)

data_velturb_local = np.zeros((n_ti_1, nz, ny, 3))
data_velturb_output_local = np.zeros((n_ti_1, ny, 3))
data_velturb_output = np.zeros((n_ti, ny, 3))
#if rank==0:
#  data_velturb = np.zeros((n_ti, nz, ny, 3))
#data_uuu_mean = np.zeros((nz, 3, 3, 3))

data_uutemp_local = np.zeros((n_ti_1, ny))
...
for i in range(n_ti_1):
  ti = ti_start + (n_ti_1*rank + i) * ti_interval
  time_local[i] = ti
  #if ti<=50000:
  ...
  fname = "POST_U_1D1_"+"{:010d}".format(ti)+"_"+"{:04d}".format(isec)+".DAT"
