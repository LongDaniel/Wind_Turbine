...
contains
  ...
  subroutine transpose_xy3(src, dst)
    ...
    if (size(src,3) > xsz(3)) then
       call decomp_abort(8, &
          'Transpose failed. Check dimension 3 of input array, should be smaller than xsz(3).') 
    end if
  ...
  subroutine transpose_yx3(src, dst)
    ...
    if (size(src,3) > ysz(3)) then
       call decomp_abort(8, &
          'Transpose failed. Check dimension 3 of input array, should be smaller than ysz(3).') 
    end if
    kmax = min(size(src,3), size(dst,3))
    if (nproc1 == 1) then
       do k=1, kmax
